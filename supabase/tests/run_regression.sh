#!/usr/bin/env bash
# Run the ACH + Score v2 regression suite against DEV Supabase.
#
# Required env var:
...
#
# Usage:
#   SUPABASE_ACCESS_TOKEN=<token> ./supabase/tests/run_regression.sh
set -euo pipefail
...
SQL_FILE="$(dirname "$0")/regression_ach_score_v2.sql"

echo "=== IOU DEV Regression Runner ==="
...
if [[ "$HTTP_STATUS" != 2* ]]; then
  ...
  echo "❌ Regression suite FAILED (HTTP $HTTP_STATUS):" >&2
  echo "$MSG" >&2
  exit 1
fi
