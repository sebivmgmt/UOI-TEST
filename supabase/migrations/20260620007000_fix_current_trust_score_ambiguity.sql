-- Corrective migration: qualify all unqualified column references in
-- get_my_current_trust_score() to eliminate the 42702 ambiguity between the
-- PL/pgSQL return-table output column `model_version` and the identically-named
-- column in score_v2_contributions.  All other unqualified references are also
-- aliased for consistency.  No formulas, return signature, grants, or security
-- attributes changed.
